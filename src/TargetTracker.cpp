...
TargetTracker::TargetTracker(ros::NodeHandle &node) : node_(node), imageTransport_(node),
  ...
  subTriggerBatch_ = node_.subscribe("/trigger_batch", 1, &TargetTracker::triggerBatchCalCallback, this);
  subDrawDiagram_ = node_.subscribe("/draw_confusion_diagram", 1, &TargetTracker::drawDiagramCallback, this);
...
//  }
  ...
  if (forwardPropagateState_) {
    ...
    if (tracking_) {
      ...
      //Publish tf messages
      tfBroadcaster_.sendTransform(tf::StampedTransform(getTfMsg(state.T_w_i_), imuMsg->header.stamp, "world", "imu"));
...
//    std::cout << "TargetTracker run took " << t_iter << " sec\n" << std::endl;
...
void TargetTracker::startTracking() {
  if (run_) {
    std::cout << "TargetTracker is already running." << std::endl;
    return;
  }
...
void TargetTracker::publish(std::shared_ptr<confusion::State> statePtr, const confusion::StateVector *stateVector) {
  ...
  auto state = std::dynamic_pointer_cast<ImuState>(statePtr);
  ...
  for (int i = 0; i < stateVector->size(); ++i) {
    auto state = std::dynamic_pointer_cast<const ImuState>((*stateVector)[i]);
